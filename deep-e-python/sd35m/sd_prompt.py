...
def main():
    ...
    import os 
    print(os.getcwd())
    parser = argparse.ArgumentParser(description='Stable Diffusion 3.5 Inference with LoRA')
    parser.add_argument('--model_path', type=str, help='Path to base model',required=True)
    parser.add_argument('--lora_path', type=str, help='Path to LoRA weights directory',required=True)
    parser.add_argument('--lora_weight', type=str, default="pytorch_lora_weights.safetensors",help='LoRA weight filename')
    ...
    parser.add_argument('--lora_scale', type=float, default=0.7,help='LoRA scale for fusion')
    ...
    pipe.load_lora_weights(args.lora_path, weight_name=args.lora_weight)
